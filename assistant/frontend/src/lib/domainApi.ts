...
async function getJobOwnerIds() {
  const companies = await getJson<BackendCompany[]>('/api/companies');

  if (!companies.length) {
    throw new Error('Aucune entreprise liee a votre compte. Completez le setup entreprise avant de creer une offre.');
  }

  const recruiters = await getJson<BackendRecruiter[]>('/api/recruiters');

  if (!recruiters.length) {
    throw new Error('Aucun recruteur lie a votre entreprise. Reconnectez-vous avec un compte recruteur/admin.');
  }
...
export async function loadCompanyMembers(): Promise<CompanyRecruiterMember[]> {
  const members = await getJson<BackendDepartmentMember[]>('/api/departments/members');
  return (members || [])
    .filter((r): r is BackendDepartmentMember & { recruiterId: string } => Boolean(r.recruiterId))
    .map((r) => ({
...
function mapInvitation(inv: BackendInvitation): CompanyInvitation {
  return {
...
export async function inviteCompanyMember(payload: {
  ...
  const created = await postJson<BackendInvitation>('/api/company-members/invitations', {
    email: payload.email,
    role: payload.role,
...
export async function cancelCompanyInvitation(invitationId: string): Promise<void> {
  await deleteJson(`/api/company-members/invitations/${invitationId}`);
}

export async function loadInvitationByToken(token: string): Promise<CompanyInvitation> {
  const invitation = await getJson<BackendInvitation>(`/api/company-members/invitations/${token}`);
...
export async function acceptCompanyInvitation(payload: {
  ...
  }>('/api/company-members/accept-invitation', payload);
}
